# Summary

[Introduction](introduction.md)

# Getting Started

- [Build and Run](getting-started/build-and-run.md)
- [Workspace Layout](getting-started/workspace-layout.md)
- [Local Development on Submodules](getting-started/submodules.md)

# Architecture

- [Overview](architecture/overview.md)
- [The Per-Frame Tick](architecture/tick-loop.md)
- [The Change-Event Stream](architecture/session-updates.md)
- [State and the GUI Bridge](architecture/gui-bridge.md)
- [History and Scoring](architecture/history-scoring.md)

# The Crates

- [foldit-core](crates/foldit-core.md)
- [foldit-desktop](crates/foldit-desktop.md)
- [foldit-web](crates/foldit-web.md)
- [foldit-gui](crates/foldit-gui.md)
- [Render Engine (viso)](crates/viso.md)
- [Molecule Model (molex)](crates/molex.md)

# Plugins and Backends

- [The Plugin Runner](plugins/runner.md)
- [Python and Native Plugins](plugins/hosts.md)
- [Plugin SDK and Protocol](plugins/sdk.md)

# Puzzles

- [Puzzle Format and Conversion](puzzles/puzzle-format.md)

# Build and Tooling

- [Quality Gates](tooling/quality-gates.md)
- [xtask Commands](tooling/xtask.md)
- [Assets and Bundling](tooling/assets-bundling.md)
- [The Frontend](tooling/frontend.md)
