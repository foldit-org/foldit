...
Unlike viso and the plugin SDK, molex is depended on as the published crates.io
crate (`0.7.1`) with no `[patch]` block, so it builds from the release even when
the `crates/molex` submodule is checked out. See
